...
function buildPortraitWorkflow(prompt, loraName = null) {
  ...
  const base = {
    "1": { "inputs": { "ckpt_name": tierModel }, "class_type": "CheckpointLoaderSimple" },
    ...
    "5": { "inputs": { "seed": Math.floor(Math.random() * 1e9), "steps": tier.steps, "cfg": tier.cfg, "sampler_name": tier.sampler, "scheduler": tier.scheduler, "denoise": 1, "model": ["1", 0], "positive": ["3", 0], "negative": ["4", 0], "latent_image": ["2", 0] }, "class_type": "KSampler" },
...
export default async function handler(req, res) {
  ...
  const { prompt, model, lora, autoLora = true, quality = 'best' } = req.body;
  ...
  try {
    ...
    const tier = QUALITY_TIERS[quality] || QUALITY_TIERS.best;
    const tierModel = tier.model;

    let workflow;
    let selectedModel;
    let qualityLabel = tier.label;
    let estimatedTime = tier.estimatedTime;
    let category;
    ...
    if (!response.ok) {
      ...
      if (appliedLora && text.includes('error')) {
        ...
        if (retryResponse.ok) {
          ...
          message: `Creating ${qualityLabel} image...`
          });
        }
      }
